...
function applyRhythmicPatternToSlot(
    ...
    while (tickWithinSlot < currentSlotDurationTicks && iterationSafety < MAX_ITER_APPLY) {
        ...
        if (nextPitch !== null && noteDurationForSubPattern >= minSensibleNoteDuration) {
            ...
            // Se nessun pattern ha generato una nota valida, o la durata è troppo breve
            // console.log("BASS APPLY_PATTERN: No valid note generated by pattern, breaking loop for this slot.");
            break; // Esci dal loop while per questo slot per evitare stalli
        }
    }
