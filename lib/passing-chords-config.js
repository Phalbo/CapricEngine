// File: lib/passing-chords-config.js
// CapricEngine - PASSING_CHORD_RULES
...
const PASSING_CHORD_RULES = [
    ...
    },
    {
        ...
        generatePassingChord: (chord1, chord2, keyInfo, helpers) => {
            ...
            const fifthBelow = helpers.getNoteName((rootIndex + 7) % 12); // Actually V of chord2
    ...
    // --- CHROMATICI E ALTERATI ---
    {
        ...
        generatePassingChord: (chord1, chord2, keyInfo, helpers) => {
            const idx1 = helpers.NOTE_NAMES.indexOf(chord1.root);
            const idx2 = helpers.NOTE_NAMES.indexOf(chord2.root);
            const passingIdx = (idx1 + ((idx2 > idx1) ? 1 : -1) + 12) % 12;
            const passingNote = helpers.getNoteName(passingIdx);
    ...
    // Nota: le regole duplicate dal file originale sono state commentate o integrate se diverse.
    // Se una regola era duplicata ma con probabilità diversa, ho mantenuto quella con la probabilità specificata nell'ultima parte del file.
    // Esempio: la regola "Tra I e V in maggiore: bII7" è già presente, qui sotto era duplicata.
    /*
    {
        ...
        durationBeats: 0.5,
        probability: 0.23 // Questa è la probabilità diversa che avevi
    },
    */
    {
    ...
    },
    {
        ...
        condition: (chord1, chord2, keyInfo, scaleType, helpers) =>
            (chord1.degree === 'II' && chord2.degree === 'III') ||
            (chord1.degree === 'III' && chord2.degree === 'IV'),
        generatePassingChord: (chord1, chord2, keyInfo, helpers) => {
            const idx1 = helpers.NOTE_NAMES.indexOf(chord1.root);
            const idx2 = helpers.NOTE_NAMES.indexOf(chord2.root);
            if (Math.abs(idx2 - idx1) === 2) { // Intervallo di un tono
                const passingIdx = (idx1 + ((idx2 > idx1) ? 1 : -1) + 12) % 12;
                const passingNote = helpers.getNoteName(passingIdx);
                // Originalmente era 'aug', ma 'dim' o 'dom7' su una nota cromatica di passaggio sono comuni.
                return helpers.normalizeChordNameToSharps(passingNote + ((Math.random() < 0.5) ? helpers.QUALITY_DEFS.dim.suffix : helpers.QUALITY_DEFS.dom7.suffix));
            } else if (Math.abs(idx2 - idx1) === 1) { // Intervallo di un semitono
                const passingIdx = (idx1 + ((idx2 > idx1) ? 1 : -1) + 12) % 12; // Che è chord2.root se ascendente, chord1.root se discendente
                const passingNote = helpers.getNoteName(passingIdx); // Nota di arrivo o partenza
                return helpers.normalizeChordNameToSharps(passingNote + helpers.QUALITY_DEFS.dim.suffix); // Spesso un dim7 sulla nota di arrivo o partenza
            }
            return helpers.normalizeChordNameToSharps(chord1.root + helpers.QUALITY_DEFS.major.suffix); // Fallback
        },
        durationBeats: 0.5,
        probability: 0.13
    },
    ...
    },
    {
        ...
        generatePassingChord: (chord1, chord2, keyInfo, helpers) => {
            const idxVI = (helpers.NOTE_NAMES.indexOf(keyInfo.root) + (scaleType === 'minor' ? 8 : 9) + 12) % 12; // VI grado (Ab in Cm, A in Cmaj)
            const VofVI = helpers.getNoteName((idxVI + 7) % 12); // Dominante di VI
            return helpers.normalizeChordNameToSharps(VofVI + helpers.QUALITY_DEFS.dom7.suffix);
        },
        durationBeats: 0.5,
        probability: 0.11
    },
    {
        ...
        condition: (chord1, chord2, keyInfo, scaleType, helpers) => {
            const idx1 = helpers.NOTE_NAMES.indexOf(chord1.root);
            const idx2 = helpers.NOTE_NAMES.indexOf(chord2.root);
            return Math.abs(idx1 - idx2) === 1 || Math.abs(idx1 - idx2) === 11; // Movimento di semitono
        },
        generatePassingChord: (chord1, chord2, keyInfo, helpers) => {
            const idx1 = helpers.NOTE_NAMES.indexOf(chord1.root);
            const idx2 = helpers.NOTE_NAMES.indexOf(chord2.root);
            let passingNoteRoot;
            // Se ascendente (es. C -> C#), il passing è C#dim. Se discendente (C# -> C), il passing è Cdim.
            // Questo crea un approccio cromatico alla nota di destinazione.
            if ((idx2 > idx1 && idx2 - idx1 === 1) || (idx1 === 11 && idx2 === 0)) { // Movimento ascendente
                passingNoteRoot = chord2.root;
            } else { // Movimento discendente
                passingNoteRoot = chord1.root; // O potrebbe essere la nota cromatica *tra* i due se il passo fosse di un tono
            }
            // Per un semitono, un dim sulla nota di arrivo (anticipata cromaticamente) o sulla partenza è comune
            // Usiamo chord1.root + dim per un approccio semplice e comune
            return helpers.normalizeChordNameToSharps(chord1.root + helpers.QUALITY_DEFS.dim.suffix);
        },
        durationBeats: 0.5,
        probability: 0.12
    }
